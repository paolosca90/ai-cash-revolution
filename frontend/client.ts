...
// MANUALLY PATCHED TO INCLUDE 'auth' SERVICE
...
export class Client {
    ...
    public readonly auth: auth.ServiceClient // <-- AGGIUNTO
    ...
    constructor(target: BaseURL, options?: ClientOptions) {
        ...
        this.auth = new auth.ServiceClient(base) // <-- AGGIUNTO
    }
...
import {
    login as api_auth_api_login,
    register as api_auth_api_register,
    getProfile as api_auth_api_getProfile
...
export namespace auth {
    export class ServiceClient {
        ...
        public async login(params: RequestType<typeof api_auth_api_login>): Promise<ResponseType<typeof api_auth_api_login>> {
            const resp = await this.baseClient.callTypedAPI(`/auth/login`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_auth_api_login>
        }

        public async register(params: RequestType<typeof api_auth_api_register>): Promise<ResponseType<typeof api_auth_api_register>> {
            const resp = await this.baseClient.callTypedAPI(`/auth/register`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_auth_api_register>
        }

        public async getProfile(params: { userId: number }): Promise<ResponseType<typeof api_auth_api_getProfile>> {
            const resp = await this.baseClient.callTypedAPI(`/auth/profile/${params.userId}`, {method: "GET", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_auth_api_getProfile>
        }
    }
}
// === FINE BLOCCO AGGIUNTO ===
