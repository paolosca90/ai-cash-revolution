...
export const getMLAnalytics = api({
    expose: true,
    method: "GET",
    path: "/ml/analytics"
}, async () => {
...
async function generateMLTrainingInsights(trainingData) {
    ...
    const confidenceCalibration = confidenceRanges.map(range => {
        const recordsInRange = trainingData.filter(r => r.predictedConfidence >= range.min && r.predictedConfidence < range.max);
...
function getMetricValue(metrics, metricType, defaultValue) {
...
export const recordPredictionAccuracy = api({
    ...
    const { tradeId, predictedDirection, actualDirection, predictedConfidence, actualProfitLoss, modelVersion, symbol, timeframe } = req;
...
export const recordMarketPattern = api({
    ...
    const { patternName, patternType, symbol, timeframe, confidence, successRate, avgProfit, patternData } = req;
    await mlDB.exec `
...
export const getMLTrainingAnalytics = api({
    expose: true,
    method: "GET",
...
function generateMLRecommendations(insights, trainingData) {
    ...
    const highConfidenceLowSuccess = insights.confidenceCalibration.find((c) => c.confidenceRange === '80-90%' && c.actualSuccessRate < 0.7);
    ...
    const symbolAccuracies = insights.accuracyBySymbol.map((s) => s.accuracy);
    ...
    const poorConditions = conditionPerformance.filter((c) => c.accuracy < 0.6);
    if (poorConditions.length > 0) {
        recommendations.push(`🌍 Poor performance in ${poorConditions.map((c) => c.condition).join(', ')} conditions - Adjust strategy`);
    }
