...
// Database connection
const db = new SQLDatabase("accounts", {
...
export const addTradingAccount = api({ method: "POST", path: "/user/trading-accounts", expose: true }, async (req) => {
    try {
      ...
      `;
        ...
        if (user.trading_accounts_connected >= user.max_trading_accounts) {
            throw APIError.failedPrecondition(`Maximum trading accounts limit reached (${user.max_trading_accounts}). Please upgrade your subscription.`);
        }
...
export const getTradingAccounts = api({ method: "GET", path: "/user/trading-accounts/:userId", expose: true }, async ({ userId }) => {
    try {
        const accounts = await db.query `
...
export const updateTradingAccount = api({ method: "PUT", path: "/user/trading-accounts/:accountId", expose: true }, async (req) => {
    try {
...
export const testConnection = api({ method: "POST", path: "/user/trading-accounts/:accountId/test", expose: true }, async (req) => {
    try {
        // Get account details
        const accounts = await db.query `
        SELECT * FROM trading_accounts 
...
export const deleteTradingAccount = api({ method: "DELETE", path: "/user/trading-accounts/:accountId", expose: true }, async ({ accountId, userId }) => {
    try {
        // Verify account ownership
        const accounts = await db.query `
        SELECT * FROM trading_accounts 
...
export const getAccountStatus = api({ method: "GET", path: "/user/trading-accounts/:accountId/status", expose: true }, async ({ accountId, userId }) => {
    try {
        const accounts = await db.query `
...
//# sourceMappingURL=trading-accounts.js.map