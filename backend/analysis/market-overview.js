...
export const getMarketOverview = api({
    expose: true,
    method: "GET",
...
async function getTopReliableAssets() {
  ...
  `;
    ...
    for (const signal of recentSignals) {
        ...
        const reliabilityScore = Math.min(100, (avgConfidence * 0.4) +
            (winRate * 100 * 0.4) +
...
async function getRelevantMarketNews() {
        ...
        if (isPositive) {
            switch (category) {
                case "Forex":
                    ...
                    break;
                case "Indici":
                    indicesBullish += weight;
                    break;
                case "Materie Prime":
                    commoditiesBullish += weight;
                    break;
                case "Crypto":
                    cryptoBullish += weight;
                    break;
            }
        }
        else {
            switch (category) {
                case "Forex":
                    ...
                    break;
                case "Indici":
                    indicesBearish += weight;
                    break;
                case "Materie Prime":
                    commoditiesBearish += weight;
                    break;
                case "Crypto":
                    cryptoBearish += weight;
                    break;
            }
        }
    }
    const getSentiment = (bullish, bearish) => {
...
function getAssetCategory(symbol) {
    ...
    if (symbol.includes("USD") || symbol.includes("EUR") || symbol.includes("GBP"))
        return "Forex";
    if (symbol.includes("US") || symbol.includes("UK") || symbol.includes("GER") || symbol.includes("SPX"))
        return "Indici";
    if (symbol.includes("XAU") || symbol.includes("XAG") || symbol.includes("CRUDE"))
        return "Materie Prime";
...
function getAssetVolatility(symbol) {
    ...
    if (highVolatility.includes(symbol))
        return "HIGH";
    if (lowVolatility.includes(symbol))
        return "LOW";
    return "MEDIUM";
}
function getRecommendation(reliabilityScore, winRate, recentPerformance) {
    if (reliabilityScore >= 85 && winRate >= 75 && recentPerformance > 50)
        return "STRONG_BUY";
    if (reliabilityScore >= 75 && winRate >= 65 && recentPerformance > 0)
        return "BUY";
    if (reliabilityScore >= 60 && winRate >= 50)
        return "HOLD";
    if (reliabilityScore < 50 || winRate < 40)
        return "SELL";
    if (reliabilityScore < 40 && recentPerformance < -100)
        return "STRONG_SELL";
    return "HOLD";
}
