export async function fetchMarketData(symbol, timeframes, mt5Config, requireRealData = false) {
    const data = {};
...
async function fetchMT5Data(symbol, timeframe, mt5Config) {
    try {
        const { host, port } = mt5Config;
        if (!host || !port) {
...
function getSpecificSymbolMappings(symbol) {
    const mappings = {
...
function calculateIndicators(open, high, low, close) {
    ...
    if (priceChange > 0.005)
        rsi = 60 + Math.random() * 15;
    else if (priceChange > 0.001)
        rsi = 52 + Math.random() * 8;
    else if (priceChange < -0.005)
        rsi = 25 + Math.random() * 15;
    else if (priceChange < -0.001)
        rsi = 40 + Math.random() * 8;
    else
