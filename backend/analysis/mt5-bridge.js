...
export async function executeMT5Order(order, mt5Config) {
    try {
...
async function tryRealExecution(order, mt5Config) {
    try {
...
async function findCorrectExecutionSymbol(symbol, host, port) {
...
function getExecutionSymbolVariations(symbol) {
    const variations = [];
...
function getPrioritySymbolMappings(symbol) {
    const mappings = {
...
function getAlternativeSymbolNames(symbol) {
    const alternatives = {
...
export async function getMT5Positions(mt5Config) {
    try {
        const { host, port } = mt5Config;
        if (!host || !port) {
...
export async function closeMT5Position(ticket, mt5Config) {
    try {
        const { host, port } = mt5Config;
        if (!host || !port) {
