...
import { TRADING_STRATEGIES } from './trading-strategies';
import { mlEngine } from '../ml/learning-engine';
export class BacktestingEngine {
    /**
     * Run comprehensive backtest with robustness testing
     */
    async runBacktest(strategy, symbol, historicalData, config) {
        ...
        const robustnessTests = await this.runRobustnessTests(strategy, symbol, inSampleData, outOfSampleData, config);
     ...
     */
    async simulateTrades(strategy, symbol, data, config) {
        const trades = [];
     ...
     */
    async generateTradingSignal(strategy, symbol, bar, index) {
        try {
     ...
     */
    checkExitConditions(position, bar, index, config) {
     ...
     */
    createTrade(position, exitSignal, exitBar, config) {
     ...
     */
    calculatePositionSize(capital, riskPercent, signal) {
     ...
     */
    async runRobustnessTests(strategy, symbol, inSampleData, outOfSampleData, config) {
    ...
    async runWalkForwardTest(strategy, symbol, inSampleData, outOfSampleData, config) {
    ...
    async runMonteCarloSimulation(strategy, symbol, data, config) {
    ...
    async runSensitivityAnalysis(strategy, symbol, data, config) {
        return {
    ...
    async runOutOfSampleTest(strategy, symbol, outOfSampleData, config) {
    ...
    calculateQuantitativeMetrics(returns) {
        if (returns.length === 0) {
            return {
                sharpeRatio: 0,
                sortinoRatio: 0,
