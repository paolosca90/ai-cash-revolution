...
(function (TradingStrategy) {
    ...
    TradingStrategy["STATISTICAL_ARBITRAGE"] = "STATISTICAL_ARBITRAGE";
    TradingStrategy["MOMENTUM_BREAKOUT"] = "MOMENTUM_BREAKOUT";
    TradingStrategy["MEAN_REVERSION"] = "MEAN_REVERSION";
    TradingStrategy["PAIRS_TRADING"] = "PAIRS_TRADING";
    TradingStrategy["ORDER_FLOW"] = "ORDER_FLOW";
    TradingStrategy["VOLATILITY_TRADING"] = "VOLATILITY_TRADING";
    TradingStrategy["TREND_FOLLOWING"] = "TREND_FOLLOWING";
    TradingStrategy["REGIME_SWITCHING"] = "REGIME_SWITCHING";
...
export function calculateStrategyTargets(strategy, currentPrice, atr, direction, symbol, spread, marketData, mlAnalysis, adaptiveParams) {
    ...
    const baseStopLoss = calculateAdvancedStopLoss(strategy, currentPrice, adjustedATR, direction, marketData, mlAnalysis);
    // Ensure stop loss is at least 3x the spread to avoid premature stops
    const stopLossDistance = Math.max(baseStopLoss, spread * 3);
    const takeProfitDistance = calculateAdvancedTakeProfit(strategy, stopLossDistance, adjustedRiskRewardRatio, marketData, mlAnalysis);
    let stopLoss;
    let takeProfit;
    if (direction === "LONG") {
 ...
 */
function calculateAdvancedStrategyScore(strategy, marketData, symbol, mlAnalysis, marketRegime, historicalPerformance) {
    const config = TRADING_STRATEGIES[strategy];
    ...
    switch (strategy) {
        ...
        case TradingStrategy.PAIRS_TRADING:
            ...
            if (!mlAnalysis?.correlationAnalysis)
                score *= 0.5;
            break;
        case TradingStrategy.ORDER_FLOW:
        ...
        case TradingStrategy.REGIME_SWITCHING:
            ...
            if (!marketRegime || marketRegime.confidence < 0.6)
                score *= 0.7;
            break;
    }
 ...
 */
export function getOptimalStrategy(marketData, symbol, userPreference, mlAnalysis, historicalPerformance) {
    ...
    for (const strategy of allStrategies) {
        ...
        let score = calculateAdvancedStrategyScore(strategy, marketData, symbol, mlAnalysis, marketRegime, historicalPerformance);
...
function isStrategyValid(strategy, marketData, symbol, mlAnalysis) {
    const config = TRADING_STRATEGIES[strategy];
    const symbolChar = getSymbolCharacteristics(symbol);
    const volatility = calculateMarketVolatility(marketData);
    const trendStrength = calculateTrendStrength(marketData);
    // Check basic requirements
    const basicValid = (volatility <= config.volatilityThreshold * 2.5 &&
 ...
 */
function calculateAdvancedStopLoss(strategy, currentPrice, atr, direction, marketData, mlAnalysis) {
    const config = TRADING_STRATEGIES[strategy];
 ...
 */
function calculateAdvancedTakeProfit(strategy, stopLossDistance, riskRewardRatio, marketData, mlAnalysis) {
 ...
 */
export function getMarketRegime(marketData, mlAnalysis) {
    const volatility = calculateMarketVolatility(marketData);
    const trendStrength = calculateTrendStrength(marketData);
 ...
 */
function estimateRegimeDuration(regimeType, marketData) {
    const baseEstimates = {
        "TRENDING": 48,
        "RANGING": 24,
        "VOLATILE": 12,
 ...
 */
export function calculateAdaptiveParams(strategy, marketData, mlAnalysis, historicalPerformance) {
 ...
 */
export function analyzeStatisticalArbitrage(marketData, symbol, mlAnalysis) {
 ...
 */
export function analyzeMomentumBreakout(marketData, symbol, mlAnalysis) {
    const data5m = marketData["5m"];
    const data15m = marketData["15m"];
    const data30m = marketData["30m"];
 ...
 */
export function analyzeOrderFlow(marketData, symbol, mlAnalysis) {
    let signal = "NEUTRAL";
    let confidence = 0;
    ...
    if (mlAnalysis?.institutionalAnalysis) {
        ...
        const bullishOBs = institutional.orderBlocks.filter((ob) => ob.type === "BULLISH");
        const bearishOBs = institutional.orderBlocks.filter((ob) => ob.type === "BEARISH");
