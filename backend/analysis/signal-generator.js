...
export const generateSignal = api({ expose: true, method: "POST", path: "/analysis/signal" }, async (req) => {
    ...
    const signal = await generateTradingSignal(symbol, marketData, indicators, strategyConfig);
...
async function determineOptimalStrategy(marketData, indicators) {
...
async function generateTradingSignal(symbol, marketData, indicators, strategy) {
...
export async function generateSignalForSymbol(symbol, mt5Config, tradeParams, strategy = "moderate") {
    ...
    const signal = await generateSignalInternal(symbol, marketData[marketData.length - 1].close, // Current price
    indicators, strategyConfig);
    return signal;
}
