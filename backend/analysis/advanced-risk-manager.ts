...
export interface RiskParameters {
  maxPortfolioRisk: number; // Maximum portfolio risk as % of capital
  maxSinglePositionRisk: number; // Maximum single position risk
  ...
  lookbackPeriod: number; // Days for volatility/correlation calculations
}
...
export interface PositionRisk {
  ...
  riskAmount: number; // Amount at risk (entry to stop loss)
  riskPercent: number; // Risk as % of portfolio
  timeInPosition: number; // Hours in position
  volatility: number; // Position volatility
  beta: number; // Position beta to portfolio
}

export interface PortfolioRisk {
  totalRisk: number; // Total portfolio risk
  concentrationRisk: number; // Concentration risk score
  correlationRisk: number; // Correlation risk score
  leverageRisk: number; // Leverage risk score
  liquidityRisk: number; // Liquidity risk score
  timeRisk: number; // Time-based risk (holding period)
  var95: number; // 95% Value at Risk
  var99: number; // 99% Value at Risk
  expectedShortfall: number; // Conditional VaR
  maxDrawdownRisk: number; // Current drawdown level
...
export interface DynamicPositionSize {
  ...
  riskAdjustment: number; // Multiplier based on current risk
  volatilityAdjustment: number; // Adjustment for volatility
  correlationAdjustment: number; // Adjustment for correlation
  marketRegimeAdjustment: number; // Adjustment for market conditions
  finalSize: number;
  reasoning: string[];
}
...
export class AdvancedRiskManager {
  ...
  private riskHistory: Array<{ timestamp: Date; portfolio_risk: PortfolioRisk }> = [];
   ...
   */
  async getRiskRecommendations(): Promise<string[]> {
  ...
  private calculateInstrumentVolatility(marketData: any): number {
    try {
  ...
  private calculateAverageCorrelation(symbol: string): number {
    ...
    if (!correlations || correlations.size <= 1) return 0;
    
    const values = Array.from(correlations.values());
    return ss.mean(values);
  }
  
  private identifyMarketRegime(marketData: any): { stress_level: 'LOW' | 'MODERATE' | 'HIGH' } {
    try {
      const volatility = this.calculateInstrumentVolatility(marketData);
      
      if (volatility > 0.40) return { stress_level: 'HIGH' };
      if (volatility > 0.25) return { stress_level: 'MODERATE' };
  ...
  private checkConcentrationLimit(symbol: string, strategy: TradingStrategy): number {
    ...
    if (sameSymbolPositions.length > 0) return 0.5;
    
    // Limit positions in same strategy
    if (sameStrategyPositions.length >= 3) return 0.7;
  ...
  private calculateLeverageRisk(positions: PositionRisk[]): number {
  ...
  private calculateTimeRisk(positions: PositionRisk[]): number {
    if (positions.length === 0) return 0;
  ...
  private calculateVaRMetrics(positions: PositionRisk[]): {
    var95: number;
    var99: number;
  ...
  private updateCorrelationMatrix(symbol: string): void {
  ...
  private getPositionCorrelation(symbol1: string, symbol2: string): number {
    return this.correlationMatrix.get(symbol1)?.get(symbol2) || 0;
  }
  
  private getZeroRiskMetrics(): PortfolioRisk {
    return {
      totalRisk: 0,
      concentrationRisk: 0,
      correlationRisk: 0,
