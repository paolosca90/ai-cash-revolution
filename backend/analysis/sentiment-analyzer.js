...
async function fetchNewsFromAdvancedSources(symbol) {
    ...
    const sources = [
        ...
        // Crypto news
        { name: "CoinDesk", type: 'news' },
        { name: "The Block", type: 'news' },
        // Top blogs/analysts
        { name: "ZeroHedge", type: 'blog' },
        { name: "Kathy Lien (BK Asset Management)", type: 'trader_opinion' },
        { name: "Plan B (Crypto Analyst)", type: 'trader_opinion' },
    ];
...
async function fetchRecentNewsFromNewsAPI(symbol) {
    try {
        ...
        const searchTerms = getSearchTermsForSymbol(symbol);
        const response = await fetch(`https://newsapi.org/v2/everything?q=${encodeURIComponent(searchTerms)}&sortBy=publishedAt&pageSize=5&language=en&apiKey=${apiKey}`);
        if (!response.ok) {
...
function createAdvancedSentimentPrompt(news, symbol) {
...
function parseGeminiSentimentResponse(text) {
    try {
 ...
 */
function generateFallbackSentiment(news, symbol) {
    let score = 0;
